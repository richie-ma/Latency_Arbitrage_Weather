...
Latency arbitrage strategies seek to exploit the time lag between price updates across different trading venues. At coarse time resolutions (e.g., 1 hour or more), correlated asset pairs typically exhibit strong statistical relationships. However, when the data is resampled at finer intervals, such as 1 minute, this correlation often deteriorates. This breakdown is not random; it reflects the microstructure inefficiencies caused by differences in data feed latency, execution delays, or exchange matching engine speeds.

By analyzing tick-level data, I plot the prices of both ETFs and E-mini futures in a whole day, 1 hour, 1 minute, and 30 seconds. I also correct outliers of the price series. Since the SPY tracks 1/10 of the E-mini futures, to enhance the visualization, I show the SPY price * 10 to ensure a closer comparision. Obviously, one can find the two price sereies are strongly correlated with identical movements in a day, as well as in an hour though there are some outliers in SPY price series. However, such a strong co-movement disappears when we look at a herzion of 1 miunte and 30 seconds. Generally one find periods where one venue updates a price while the other lags behind. These brief, transient decouplings of price allow arbitrageurs to buy the underpriced asset on the slower venue and simultaneously sell it on the faster one, capturing the spread before the prices realign. Such opportunities typically vanish in under a second, but are detectable through careful correlation analysis at the sub-second level. The degradation of correlation at sub-second, therefore, is not merely noise — it is the signal of inefficiency in price discovery across fragmented markets, and the entry point for latency-based alpha generation.
...
A key limitation of this project is the absence of regulatory or account-level data to directly identify HFTs. As a result, I rely on precipitation disruptions as a proxy for HFT speed advantages. This limitation constrains the ability to attribute observed market dynamics exclusively to HFT activity. Future projects could use regulatory data from both futures and securities markets to precisely identify HFTs and better estimate their effects on market liquidity.
